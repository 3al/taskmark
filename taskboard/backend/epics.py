...
def epic_tasks(tasks_dir: Path, key: str, pipeline) -> list[dict]:
    ...
    Частный случай среза по полю (`task_slices.field_tasks`): состав эпика
    читают как маршрут, по которому эпик едет. Пустой ключ не собирает задачи
    без эпика: `epic: ~` — это «эпика нет», а не эпик с пустым именем.
    """
    return field_tasks(tasks_dir, "epic", key, pipeline)
