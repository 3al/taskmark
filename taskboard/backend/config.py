...
DEFAULTS: dict = {
    ...
    # светится «здесь работают сейчас». Четверть часа — примерно один шаг
    # работы агента; ноль выключает подсветку целиком, поэтому отдельного
    # флага показа у неё нет
    "card_fresh_minutes": 15,
