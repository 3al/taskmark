...
DEFAULTS: dict = {
    ...
    # Сколько секунд висит всплывашка. Шесть — время прочесть две строки и
    # вернуться к работе; ноль означает «не гасить само», и тогда уведомление
    # ждёт закрытия крестиком. Наведение мыши таймер приостанавливает при любом
    # значении, поэтому увеличивать его ради «не успел прочесть» не нужно
    "notice_seconds": 6,
    # Через сколько минут простоя терминала Claude Code звать человека. Среда
    # сообщает о простое примерно через минуту после конца реплики — человек
    # в это время ещё читает ответ. Три минуты отделяют «читает» от «отошёл»;
    # ноль зовёт сразу. Читает её хук среды прямо из глобального конфига
    "notice_idle_minutes": 3,
    # Выключенные источники уведомлений: {вид: false}. Хранятся только
    # отключённые — источник, которого в словаре нет, говорит. Иначе новый вид
    # молчал бы у всех, кто сохранил настройки до его появления. Настройка
    # глобальная: показывать ли всплывашку — свойство человека, а не репозитория
    "notice_sources": {},
    # Источники, чьи уведомления ждут закрытия вместо таймера: {вид: true}.
    # Хранятся только отличия от умолчания поставки — по той же причине, что и
    # выключатели источников. Пусто: всё тает само, ждать крестика человек
    # просит сам, когда не успевает прочесть стопку
    "notice_sticky": {},
    # Источники, чьи уведомления звучат: {вид: true}. Хранятся только
    # включённые, потому что умолчание — тишина: пришедшее с обновлением
    # обязано молчать, пока человек сам не попросит звук
    "notice_sound": {},
    # Громкость звука уведомлений в процентах, общая для всех источников.
    # Половина: сигнал должен быть слышен рядом с клавиатурой и не пугать
    # того, кто включил звук первый раз
    "notice_volume": 50,
