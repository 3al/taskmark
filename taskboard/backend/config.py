...
DEFAULTS: dict = {
    ...
    "card_stale_days": 7,
    # Подсветка свежести: сколько минут после правки файла задачи её карточка
    # светится «здесь работают сейчас». Минута — столько, сколько правка и
    # означает «сейчас»: за один шаг агент трогает несколько задач, и порог
    # пошире засветил бы половину доски, включая уже отпущенное. Ноль выключает
    # подсветку целиком, поэтому отдельного флага показа у неё нет
    "card_fresh_minutes": 1,
    ...
    # Задачи из чата Telegram. Выключено по умолчанию: это второе место, где
    # инструмент ходит в сеть, и включать его человек должен осознанно.
    # Ключи глобальные, а не проектные: токен бота и привязка чатов — свойство
    ...
    "telegram_proxy": "",
    # Свой адрес Bot API: реверс-прокси на своём домене или локальный
    # telegram-bot-api. Пусто — сам телеграм
    "telegram_api_root": "",
    # Свой ник в телеграме: по нему бот отличает адресованное себе от чужого.
    # Знает его только человек — угадать нельзя
    "telegram_username": "",
    # Слово-хэштег, по которому сообщение считается задачей. Настройка, а не
    # константа: инструмент не привязан ни к одной команде и ни к одному языку
    "telegram_tag": "задача",
    # Привязка «чат → проект»: {"<chat_id>": "имя"} или {"<chat_id>": ["имя",
    # "имя"]} — первый по умолчанию, остальные доступны суффиксом хэштега.
    # Чат один, проектов у человека десяток: выбор делается один раз здесь,
...
CARD_LIMITS: dict[str, tuple[int, int]] = {
    ...
    # Ноль выключает возраст целиком — как у соседней настройки свежести
    "card_stale_days": (0, 365),
    # Свежесть — единственное поле вида карточки, где ноль осмыслен: он и есть
    # выключатель подсветки. Верх — сутки: дольше это уже не «сейчас»
    "card_fresh_minutes": (0, 1440),
}
