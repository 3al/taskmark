...
def api_board() -> dict:
    ...
    # Возраст в статусе берётся из строки доски, а до первого перехода — из
    # created файла задачи. Залежалость проверяется ещё и по времени правки
    # файла: порог считает бэкенд, превью получает уже готовый ответ
...
def _startup() -> None:
    ...
    start_watcher()
    # Запись автозагрузки прежних версий не задавала рабочую папку и заводила
    # проект в системной папке Windows при каждом входе в систему (TASK-233).
    # Ведёт она сюда же, поэтому `stale` о ней молчит — чиним сами
    # …под защитой: автозапуск — удобство, и его сбой не повод не пустить
    # человека к доске. Незакрытое исключение здесь роняет старт сервера
