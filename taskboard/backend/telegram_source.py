...
def _message_of(raw: dict) -> dict | None:
    ...
    задевать разбор. У картинки и файла текст приходит подписью (`caption`) и
    дальше едет тем же `text`. Апдейты без текста (вход в группу, картинка без
    подписи, стикер) отбрасываются здесь же — верхним слоям они не нужны.
    """
