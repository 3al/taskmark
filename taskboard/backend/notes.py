"""Заметки агента: запись строки в хронологию файла задачи.
...
`check_task_file`, который следит за порядком заметок.
"""
...
def append_note(task_path, text: str, author: str = BOARD_AUTHOR) -> str | None:
    """Дописать заметку в конец секции «Заметки агента». Вернуть строку.

    None — файл не читается или в нём нет ни секции заметок, ни места для неё:
    молча портить структуру чужого файла нельзя.
    """
    path = Path(task_path)
    text = _one_line(text)
    if not text:
    ...
    start = next((i for i, ln in enumerate(lines)
                  if ln.strip() == NOTES_SECTION), None)
    if start is None:
