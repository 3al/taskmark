...
# Эпик задачи: чем она является частью. Смена уходит в хронологию по той же
# причине, что тип и размер, — во frontmatter видно только нынешнее значение
EPIC_TEXT = "эпик: {now} (было {was})"
SIZE_TEXT = "размер: {now} (было {was})"
# Исполнитель: кто занимается задачей на этапе проверки. Во frontmatter видно
# только нынешнее имя, а «кому передали и когда» — как раз то, ради чего поле
# и заводили: без строки в хронологии передача не отличается от опечатки
ASSIGNEE_TEXT = "исполнитель: {now} (было {was})"
# Автор: кто задачу принёс. Проставляется при заведении и дальше не меняется —
# но если его всё же поправили, это событие: «принёс» отвечает на вопрос
# происхождения, и молча переписанный ответ хуже неверного
AUTHOR_TEXT = "автор: {now} (было {was})"
...
# Тот же источник, но мимо скилла момента: подпись отличает ручной ход от
# прохода через скилл. Зеркало константы в `templates/tasks/set_status.py`
...
#
# Содержимое скобок у подписи скрипта не разбирается: имя модели задаёт человек,
# и скобки внутри него законны («opencode (kimi-code/k3)»). Границу держит
# разделитель ` · `, исключённый из класса, — большего разбору и не нужно.
TRANSITION_RE = re.compile(
    r"^- \*\*(?P<stamp>\d{4}-\d{2}-\d{2} \d{2}:\d{2})\*\* · "
    rf"(?P<author>{BOARD_AUTHOR}|{SCRIPT_SOURCE}(?: вручную)?(?: \([^·]+\))?) · "
    r"(?P<from>[^·→,:«»]+) → (?P<to>[^·→,:«»]+)$")
