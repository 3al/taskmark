...
# (нет части, не хватает элементов, устарело, лишнее). Пустая строка —
# состояния у части не бывает: реестр ENV_PARTS в этом случае не даёт и кода
_PART_WORDING = {
    ...
    "status_script": ("Нет {names} — инструмент агента для управления задачами", "",
                      "{names} устарел — не поддерживает актуальные возможности", ""),
    "notify_script": ("Нет {names} — агенту нечем позвать человека к доске", "",
                      "{names} устарел — не поддерживает актуальные возможности", ""),
    ...
    "skills": ("Скиллы не развёрнуты — агент не знает процесса работы с задачами",
               ...
               "Скиллы выключенных возможностей остались в проекте: {names}"),
    "commands": ("Команды opencode не развёрнуты: без них среда не видит скиллы",
                 ...
                 "Команды opencode выключенных возможностей остались: {names}"),
    # Часть общая, обработчиков в ней со временем будет больше одного, поэтому
    # текст называет класс, а не сегодняшний единственный хук. Пример нужен,
    # чтобы человек понимал, что даст кнопка
    "hooks": ("Хуки не развёрнуты (подсказки агенту в момент действия) — "
              "например, что коммит по задаче в работе означает её передачу",
              "Не хватает хуков: {names}",
              "Хуки устарели: {names}", ""),
    "hook_registration": ("Хуки не подключены — среда не знает про них, "
                          "пока на обработчик не сослались в {names}", "", "", ""),
    "rules": ("Правила для агентов не развёрнуты: {names}", "",
              "Правила для агентов устарели: {names}",
              "Рядом с правилами для агентов осталась старая секция Task Management: "
              "{names}"),
